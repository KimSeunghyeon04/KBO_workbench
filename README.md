...
상단 명령 바에서 범위를 실행하고, 왼쪽 고정 높이 가상 목록에서 경기를 고르면 오른쪽 상세에서
finding·이전 사본·revision과 다음 행동을 확인한다. 과거 작업은 별도 `활동 기록` drawer에서 열며
페이지 아래로 이어 붙이지 않는다. 900px 이하에서는 목록과 상세를 동시에 쌓지 않고 선택 시 상세로
전환하며 `목록으로`로 돌아간다.
...
`/record-corrections`는 KBO 기록정정현황을 수집해 기본 `미처리` 큐에 적용 가능·후보 확인·DB 경기
없음만 표시하고, 이미 반영·지원 범위 외·해결·무시는 `완료·제외` 큐로 분리한다. 목록 API는
...
DB catalog는 경기 날짜·팀·current revision·전체 revision 수를 한 번에 제공한다. 수집·기록정정·
Database·재생의 긴 목록은 viewport와 overscan 구간만 렌더링하고, Database의 전체 revision hash
