...
분석 snapshot, 모델 검증·학습·조회, worker 실행기와 화면 조회의 책임을 분리한 구조 및
검증 범위는 [모듈화 점검 기록](docs/reviews/2026-09-22-modularity-refactor.md)에 정리했다.
DB 원장 복원·재생 변환, 파일 current 전환·복구, 보정 세션 수명 관리의 내부 경계는
[저장·세션 책임 분리](docs/reviews/2026-09-23-persistence-boundaries.md)를 따른다.
기록정정 저장소는 수집 작업·원문 시즌 적재·검토 상태를 분리하고, 보정 drawer는 이벤트 종류별
입력 UI를 별도 모듈에 둔다. DB 경기를 보정 초안으로 다시 열 때는 공용 worker에서 한 번 전체
컴파일한 뒤 workspace가 ready/quarantine을 결정한다. 컴파일 실패 시 기존 current는 유지한다.
기록정정 타석 매칭도 공용 worker에서 수행하며, 평가 중 읽은 원장을 제안에 재사용하고 저장 직전
DB revision·hash를 다시 확인한다. workspace의 원문 저장·시작 무결성 검증과 보정 화면의
finding·타임라인·상세·원본 비교는 각각의 모듈이 맡는다.
최초 원장·finding 보관과 교정·수집 저널의 파일 처리도 내부 store로 분리한다. workspace는 잠금과
교정 base 검증, 저장 분류와 복구 순서를 조정하며 공개 API와 저장 형식을 유지한다.
...
선수 분석과 경기 재생은 분석에 있다. 현재 탭에서 영역별 마지막 주소와 선택 조건을 기억한다.
처음 분석을 열면 `/analysis/players`의 **선수 분석**에서 시작한다. `/analysis`도 같은 선수 목록을
표시한다. 시즌·투수/타자를 선택하고 이름 또는 ID로 검색하며, 실제 투구가 기록된 선수를 48명씩
표시한다. 동명이인은 카드의 ID로 구별한다.

- 투수를 선택하면 **코스·결정구**를 열고, 같은 선수의 **기본 기록**, **구질·움직임**, **최근 변화**,
  **배합·궤적**, **등판·운용**, **타자와 매치업**으로 이동한다.
- 타자를 선택하면 **반응·성적**을 열고 **기본 기록**, **선구안**, **투수와 매치업**으로 이동한다.
- 사이드 메뉴의 **리그·경기**에는 선수·팀 성적, 주루와 추가 진루, 구장 득점 환경, 경기 재생을 둔다.
- **분석 관리**에는 분석 자료 품질과 분석 모델 관리를 둔다.

선수별 항목 이동은 선수·시즌·경기 종류·기간을 유지한다. `선수 변경`은 같은 범위의 목록으로
돌아간다. 선수 탐색은 확인된 정규시즌이 기본이며, 기존 구질·선구안 주소를 직접 열 때의 수집 경기
전체 기본값도 유지한다. 검색과 페이지 이동은 받은 목록을 사용하고, 공통 선수 바는 기존 화면의
목록 캐시만 관찰한다. 자세한 경계는 [선수 중심 탐색](docs/reviews/2026-09-22-player-centered-navigation.md)에 있다.
작은 화면에서는 메뉴를 펼치고 스크롤해 이동하며 Escape로 닫을 수 있다.
[UI 점검과 개선 기록](docs/reviews/2026-09-22-frontend-ui.md)에 키보드 조작의 변경을 정리했다.

선수 리포트 머리글은 선수·시즌·조회 범위와 catalog의 범위 내 투구 수를 표시한다. 이 수치는
카운트·구종 등 상황 조건을 적용한 아래 결과 표본과 구분한다. 코스·결정구와 타자 반응 화면은
빠른 좌우 타석·초구·2스트라이크 조건, 핵심 지표, 상황별 표와 지도로 읽는다. 조건 초기화는
선수와 조회 범위를 유지한다. 구종·카운트·타석별 표를 전환하고 열 제목으로 정렬하며 `상세 지표`와
`지표 안내`에서 전체 열과 분모를 확인한다. 1,500px 이상에서는 표와 지도를 나란히 표시한다.
기본 조건을 먼저 표시하고 `세부 조건`에서 기간 등을 조정하며 투구·타석 근거와 재생 링크는
펼쳐서 확인한다. [공식 야구 사이트를 참고한 UX 개선](docs/reviews/2026-09-23-reference-analytics-ux.md)과
[앞선 분석 화면 재설계](docs/reviews/2026-09-22-analysis-design.md)에 구조와 검증을 정리했다.

**기본 기록**(`/analysis/player-statistics`)에서는 선택한 선수의 경기·타석·타수·안타·홈런과
AVG/OBP/SLG/OPS, 또는 등판·이닝·상대 타자·피안타·사사구·삼진·실점·ERA를 확인한다.
선수·시즌·경기 종류·기간을 유지하며 이적 기록은 당시 팀별로 구분한다. 자책점 미제공은 0으로
채우지 않고 확인된 경기 범위를 함께 표시한다. [조회와 UI 경계](docs/reviews/2026-09-23-basic-records.md)에
구현을, [KBO 공개 기록 표본 대조](docs/analysis/2026-09-23-public-records-audit.md)에 별도 조사 결과를
정리했다. 공개 기록 비교는 화면 기능이 아니며 운영 기록을 자동 교정하지 않는다.

수집 시 기록지의 이닝별 타격 결과가 명시적인 안타·홈런 합계와 일치하면 2루타·3루타도
공식 비교 항목에 포함한다. 중계와 다른 장타 기록, 또는 공식 기록에 양수 실적이 있는 선수의
계산 행 누락은 import를 차단한다. 해석할 수 없는 기록지와 미판정 타점은 경고로 남겨 검토한다.
[전체 선수 대조](docs/analysis/2026-09-24-all-player-records-audit.md)와
[확인된 오류 교정](docs/analysis/2026-09-24-record-repair.md)에 근거와 검증 결과를 정리했다.
...
투수 코스/변화/배합/운용·타자 프로필·매치업은 선수를 선택한 뒤 분석 항목에서 이동한다.
성적·주루·구장 환경은 리그·경기, 품질·모델 관리는 분석 관리에서 연다. 기존 구질/선구안 주소에
경기 종류를 지정하지 않으면 수집 경기 전체가 기본이며 새 통계는 확인된 정규시즌이 기본이다.
선수 탐색에서 넘어온 범위는 구질/선구안에도 그대로 적용한다. 기간·경기 종류와 지표별 분모를
확인하고 근거 경기의 정확한 revision으로 재생한다.
...
선수 분석에서 타자를 선택한 뒤 **선구안**(`/analysis/batter-discipline`)에서 시즌·카운트·구종·타석을
조정한다. 실제 코스별
...
화면의 `지금 동기화`로 보유한 sealed 경기 시즌을 수동 확인할 수도 있다. 시즌을 지정하지 않은
수동·예약 동기화는 공식 기록정정 공지가 시작된 2022년 이후의 보유 시즌만 조회한다. 제도 이전인
2020~2021년을 명시적으로 요청하면 공식 연도 control 검증에서 거부하며, 빈 정상 수집으로
처리하지 않는다.
제도는 2022년 4월 29일 신설이 발표됐고, 5월 17일 경기부터 적용됐다
...
비식별화해 회귀 테스트한다. 공지 통계 괄호 안의 항목별 줄바꿈과 닫는 괄호 뒤 쉼표를 처리하며
원문과 출처는 보존한다. 닫히지 않은 괄호나 모호한 선수·팀 경계는 임의로 연결하지 않는다.
...
Graft 최초 색인과 큰 변경 후 갱신은 `pnpm graft:build`로 실행한다. 앱·패키지·테스트·운영 스크립트와
루트 코드 설정만 포함하며, 이 범위는 이후 Graft 조회의 자동 갱신에도 유지된다. Git 조회가 실패해도
`test-results`의 과거 복사본과 `analysis`의 연구 산출물이 서비스 코드의 호출 관계에 섞이지 않는다.
