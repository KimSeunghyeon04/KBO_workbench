...
CREATE TABLE workbench.contract_metadata (
  ...
  analytics_contract_version INTEGER NOT NULL CHECK (analytics_contract_version = 2),
  projection_version INTEGER NOT NULL CHECK (projection_version = 2)
);
INSERT INTO workbench.contract_metadata VALUES (TRUE, 2, 2);
...
CREATE TABLE workbench.game_teams (
...
CREATE TABLE workbench.game_rosters (
  game_id TEXT NOT NULL, revision INTEGER NOT NULL, side TEXT NOT NULL CHECK (side IN ('away','home')),
  roster_index INTEGER NOT NULL CHECK (roster_index >= 0), player_id TEXT NOT NULL, player_name TEXT NOT NULL,
...
CREATE TABLE workbench.game_roster_positions (
  ...
  FOREIGN KEY (game_id, revision, side, roster_index)
    REFERENCES workbench.game_rosters ON DELETE CASCADE
);
...
CREATE TABLE workbench.tracking_candidates (
  game_id TEXT NOT NULL, revision INTEGER NOT NULL,
  ...
  FOREIGN KEY (game_id, revision, canonical_tracking_id)
    REFERENCES workbench.tracking_candidates(game_id, revision, tracking_id) DEFERRABLE INITIALLY DEFERRED,
...
CREATE TABLE analytics.play_facts (
...
CREATE TABLE analytics.play_events (
  game_id TEXT NOT NULL, revision INTEGER NOT NULL, play_sequence INTEGER NOT NULL,
  ...
  FOREIGN KEY (game_id, revision, play_sequence) REFERENCES analytics.play_facts ON DELETE CASCADE,
...
CREATE TABLE analytics.runner_movement_facts (
  game_id TEXT NOT NULL, revision INTEGER NOT NULL, play_sequence INTEGER NOT NULL,
...
CREATE TABLE analytics.plate_appearance_facts (
...
CREATE TABLE analytics.plate_appearance_events (
  game_id TEXT NOT NULL, revision INTEGER NOT NULL, plate_appearance_index INTEGER NOT NULL,
  ...
  FOREIGN KEY (game_id, revision, plate_appearance_index)
    REFERENCES analytics.plate_appearance_facts ON DELETE CASCADE,
  FOREIGN KEY (game_id, revision, event_sequence, event_id)
    REFERENCES workbench.relay_event_facts(game_id, revision, event_sequence, event_id)
);

CREATE TABLE analytics.pitch_facts (
...
CREATE TABLE analytics.pitch_tracking (
  game_id TEXT NOT NULL, revision INTEGER NOT NULL, tracking_sequence INTEGER NOT NULL,
  pitch_id TEXT NOT NULL,
  tracking_id TEXT NOT NULL, source_pitch_id TEXT NULL, source_pitch_ordinal INTEGER NOT NULL,
  source_endpoint TEXT NOT NULL, source_block_index INTEGER NOT NULL, source_row_index INTEGER NOT NULL,
  pitcher_id TEXT NULL, batter_id TEXT NULL, observed_at TEXT NULL,
  stance TEXT NULL CHECK (stance IN ('L','R','S')), x0 DOUBLE PRECISION NULL,
  y0 DOUBLE PRECISION NULL, z0 DOUBLE PRECISION NULL, vx0 DOUBLE PRECISION NULL,
  vy0 DOUBLE PRECISION NULL, vz0 DOUBLE PRECISION NULL, ax DOUBLE PRECISION NULL,
  ay DOUBLE PRECISION NULL, az DOUBLE PRECISION NULL, cross_plate_x DOUBLE PRECISION NULL,
  cross_plate_y DOUBLE PRECISION NULL, top_sz DOUBLE PRECISION NULL, bottom_sz DOUBLE PRECISION NULL,
  PRIMARY KEY (game_id, revision, pitch_id), UNIQUE (game_id, revision, tracking_id),
  UNIQUE (game_id, revision, tracking_sequence),
  FOREIGN KEY (game_id, revision, pitch_id) REFERENCES analytics.pitch_facts(game_id, revision, pitch_id),
  CHECK (bottom_sz IS NULL OR top_sz IS NULL OR bottom_sz < top_sz)
);

CREATE TABLE analytics.game_final_states (
  game_id TEXT NOT NULL, revision INTEGER NOT NULL,
...
CREATE TABLE analytics.batter_game_facts (
...
CREATE TABLE analytics.pitcher_game_facts (
  game_id TEXT NOT NULL, revision INTEGER NOT NULL, line_index INTEGER NOT NULL,
  player_id TEXT NOT NULL, side TEXT NOT NULL CHECK (side IN ('away','home')),
  batters_faced INTEGER NOT NULL, outs_recorded INTEGER NOT NULL, hits INTEGER NOT NULL,
...
CREATE TABLE analytics.baserunner_game_facts (
  game_id TEXT NOT NULL, revision INTEGER NOT NULL, line_index INTEGER NOT NULL,
  player_id TEXT NOT NULL, side TEXT NOT NULL CHECK (side IN ('away','home')),
...
CREATE INDEX pitch_pitcher_date_idx ON analytics.pitch_facts (pitcher_id, game_id, revision, pitch_call);
CREATE INDEX pitch_batter_date_idx ON analytics.pitch_facts (batter_id, game_id, revision, pitch_call);
CREATE INDEX pitch_source_id_idx ON analytics.pitch_facts (source_pitch_id, game_id, revision);
CREATE INDEX pa_batter_result_idx ON analytics.plate_appearance_facts (batter_id, result, game_id, revision);
CREATE INDEX pa_pitcher_result_idx ON analytics.plate_appearance_facts (pitcher_id, result, game_id, revision);
CREATE INDEX movement_runner_idx ON analytics.runner_movement_facts (runner_id, reason, game_id, revision);
...
SELECT f.* FROM analytics.play_facts f
...
SELECT e.* FROM analytics.play_events e
...
SELECT f.* FROM analytics.runner_movement_facts f
JOIN workbench.games g ON g.game_id=f.game_id AND g.current_revision=f.revision;
CREATE VIEW analytics.current_plate_appearances AS
SELECT f.* FROM analytics.plate_appearance_facts f
JOIN workbench.games g ON g.game_id=f.game_id AND g.current_revision=f.revision;
...
LEFT JOIN workbench.game_rosters pitcher
...
LEFT JOIN analytics.pitch_tracking t
  ON t.game_id=p.game_id AND t.revision=p.revision AND t.pitch_id=p.pitch_id;
CREATE VIEW analytics.current_player_game_batting AS
SELECT f.* FROM analytics.batter_game_facts f
JOIN workbench.games g ON g.game_id=f.game_id AND g.current_revision=f.revision;
CREATE VIEW analytics.current_player_game_pitching AS
SELECT f.* FROM analytics.pitcher_game_facts f
JOIN workbench.games g ON g.game_id=f.game_id AND g.current_revision=f.revision;
CREATE VIEW analytics.current_player_game_baserunning AS
SELECT f.* FROM analytics.baserunner_game_facts f
JOIN workbench.games g ON g.game_id=f.game_id AND g.current_revision=f.revision;
CREATE VIEW analytics.all_revision_pitches AS SELECT * FROM analytics.pitch_facts;
CREATE VIEW analytics.all_revision_plate_appearances AS SELECT * FROM analytics.plate_appearance_facts;
CREATE VIEW analytics.all_revision_plays AS SELECT * FROM analytics.play_facts;
CREATE VIEW analytics.all_revision_runner_movements AS SELECT * FROM analytics.runner_movement_facts;
CREATE VIEW analytics.all_revision_player_game_batting AS SELECT * FROM analytics.batter_game_facts;
CREATE VIEW analytics.all_revision_player_game_pitching AS SELECT * FROM analytics.pitcher_game_facts;
CREATE VIEW analytics.all_revision_player_game_baserunning AS SELECT * FROM analytics.baserunner_game_facts;

CREATE VIEW analytics.current_player_season_batting AS
SELECT r.season, f.player_id,
...
JOIN analytics.current_game_revisions r USING (game_id,revision)
GROUP BY r.season,f.player_id;
CREATE VIEW analytics.current_player_season_pitching AS
WITH game_lines AS (
  SELECT r.season, f.player_id, SUM(f.batters_faced)::BIGINT batters_faced,
  ...
  GROUP BY p.season,p.pitcher_id
)
SELECT g.season,g.player_id,g.batters_faced,g.outs_recorded,g.strikeouts,g.walks,
...
FROM game_lines g LEFT JOIN pitch_lines p USING (season,player_id);
CREATE VIEW analytics.current_player_season_baserunning AS
SELECT r.season, f.player_id, SUM(f.advances)::BIGINT advances,
...
BEGIN
  FOREACH table_ref IN ARRAY ARRAY[
    'workbench.game_teams','workbench.game_rosters','workbench.game_roster_positions',
    ...
    'analytics.play_facts','analytics.play_events','analytics.runner_movement_facts',
    'analytics.plate_appearance_facts','analytics.plate_appearance_events','analytics.pitch_facts',
    'analytics.pitch_tracking','analytics.game_final_states','analytics.batter_game_facts',
    'analytics.pitcher_game_facts','analytics.baserunner_game_facts'
  ] LOOP
    EXECUTE format(
